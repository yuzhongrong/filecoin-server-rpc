...
func (b *Bitcoind) DecodeRawTransaction(hexstring string) (rawTx interface{}, err error) {

	r, err := b.client.call("decoderawtransaction", []interface{}{hexstring})
	if err = handleError(err, &r); err != nil {
		return
	}
...
// [{\"txid\":\"fb9bd2df3cef0abd9f444971dff097790b7bf146843a752cb48461418d3c7e67\",\"vout\":0}]
/*
	transactions（字符串，必备）一个由 json 对象构成的 json 数组
	outputs（字符串，必备）一个输出的 json 对象
	locktime（数字，可选，默认为 0）原始锁定时间。非 0 值也可以激活输入的锁定时间
...
*/
func (b *Bitcoind) CreateRawTransaction(inputs []TransactionInput, address string, amount uint32, lockTime int64) (hexstring string, err error) {
	// "[{\"txid\":\"fb9bd2df3cef0abd9f444971dff097790b7bf146843a752cb48461418d3c7e67\",\"vout\":0}]" "{\"1Mcg7MDBD38sSScsX3USbsCnkcMbPnLyTV\":0.01}"
	...
	r, err := b.client.call("createrawtransaction", []interface{}{inputs, outputs, lockTime})
	if err = handleError(err, &r); err != nil {
		fmt.Println(err)
		return
	}
...
// 构造一笔交易并发送流程：（1->2->3）
// 1.使用 createrawtransaction 创建一笔原始交易，注意找零。
// 1.5.（可选）若创建原始交易时为指定找零，使用 fundrawtransaction 增加找零输出。
// 2.使用 signrawtransaction 对创建的原始交易进行签名。
// 3.使用该命令提交完成签名的原始交易（放入本地节点的内存池并进行交易广播）。

// 1.hexstring（字符串，必备）原始交易的 16 进制字符串。
// 2.allowhighfees（布尔型，可选，默认为 false）允许交易费超额。
func (b *Bitcoind) SendRawTransaction(hexString string) (txID string, err error) {
	allowHighFees := false
	r, err := b.client.call("sendrawtransaction", []interface{}{hexString, allowHighFees})
	if err = handleError(err, &r); err != nil {
		return
	}
	err = json.Unmarshal(r.Result, &txID)
